...
def _fetch_synced_lyrics(title: str, artist: str) -> Optional[list]:
...
class _MiniPlayerManager:
    ...
    def _apply_track(self, title: str, artist: str) -> None:
        ...
        # new track: reset progress display
        self._progress_ms = 0
        ...
        try:
            ...
            self._time_total_label.configure(text=_fmt_time(0))
    ...
    def _start_lyric_fetch(self, title: str, artist: str) -> None:
        ...
        def worker():
            lines = _fetch_synced_lyrics(title, artist)
            if self._root is not None:
